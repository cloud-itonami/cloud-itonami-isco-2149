(ns geneng.actor
  "GenengActor — the ISCO-08 2149 general engineering professionals actor as
  a `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one general engineering
  ...
  The unconditional invariant: the GenengAdvisor can never directly
  issue a certified engineering design, certify safety compliance, or
  act against the GenengGovernor's refusal — every commit-record!
...
(defn build-graph
  "Build a compiled GenengActor graph. `store` implements
  `geneng.store/Store`. `advisor` implements
  `geneng.advisor/Advisor` (defaults to `mock-advisor`).
...
(defn run-request!
  "Run one general engineering operation request to completion or
